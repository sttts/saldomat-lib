/***************************************************************************
 begin       : Mon Mar 01 2004
 copyright   : (C) 2004 by Martin Preuss
 email       : martin@libchipcard.de

 ***************************************************************************
 *          Please see toplevel file COPYING for license details           *
 ***************************************************************************/


#ifndef AQBANKING_VERSION_H
#define AQBANKING_VERSION_H

#define AQBANKING_VERSION_MAJOR 5
#define AQBANKING_VERSION_MINOR 5
#define AQBANKING_VERSION_PATCHLEVEL 0
#define AQBANKING_VERSION_BUILD 2
#define AQBANKING_VERSION_TAG "git"
#define AQBANKING_VERSION_FULL_STRING "5.5.0.2"
#define AQBANKING_VERSION_STRING "5.5.0"


#define AQBANKING_SO_CURRENT 39
#define AQBANKING_SO_REVISION 2
#define AQBANKING_SO_AGE 5
#define AQBANKING_SO_EFFECTIVE 34

#endif /* AQBANKING_VERSION_H */


